...
func TestSignup(t *testing.T) {
	...
	t.Run("Email and Password Required", func(t *testing.T) {
		...
		mockUserService.On("Signup", mock.AnythingOfType("backgroundCtx"), mock.AnythingOfType("*model.User")).Return(nil)
	...
	t.Run("Password too lang", func(t *testing.T) {
		// We just want this to show that it's not called in this case
		mockUserService := new(mocks.MockUserService)
		mockUserService.On("Signup", mock.AnythingOfType("backgroundCtx"), mock.AnythingOfType("*model.User")).Return(nil)
		...
		reqBody, err := json.Marshal(gin.H{
			"email":    "bob@bob.com",
			"password": "supersecret1234567890123456789hhhelahcohlhqeihoh",
		})
		assert.NoError(t, err)
	...
	t.Run("Error return from UserService", func(t *testing.T) {
		u := &model.User{
			Email:    "bob@bob.com",
		...
		mockUserService := new(mocks.MockUserService)
		mockUserService.On("Signup", mock.AnythingOfType("backgroundCtx"), u).Return(apperrors.NewConflict("User Alerady Exists", u.Email))
	...
	t.Run("Successful Token Creation", func(t *testing.T) {
		...
		mockUserService.
			On("Signup", mock.AnythingOfType("backgroundCtx"), u).
			Return(nil)
		mockTokenService.
			On("NewPairFromUser", mock.AnythingOfType("backgroundCtx"), u, "").
